...
| KR-D-060 | Decidido | El adapter Markdown normalizará `\ddagger` al símbolo Unicode `‡`. Los elementos HTML `ol` y `ul` del detalle de notas recibirán `1em` de margen y `1em` de padding interno para proteger sus marcadores sin alterar calendario ni la cabecera ni reducir el ancho global del `TextViewer`. | Algunos tipos de letra del lector no representan bien el comando crudo y los marcadores externos de listas pueden quedar apenas fuera del área visible. |

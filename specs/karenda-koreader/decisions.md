...
| KR-D-057 | Decidido | El detalle de notas usará `TextViewer` en modo HTML con un fragmento generado por el parser Markdown de KOReader. El adapter eliminará HTML, imágenes y enlaces inseguros de la entrada antes de convertir; solo insertará etiquetas controladas para tablas GFM básicas, cursiva matemática, exponentes, subíndices, bloques de fórmula y etiquetas de flechas parametrizadas. | Reutiliza el scroll, selección, paginación y tipografía nativos sin incorporar un navegador ni KaTeX al Kindle, pero conserva la jerarquía visual de Markdown y evita ejecutar contenido del snapshot. |

...
| KR-REQ-009 | Mapper RFC 3339 versus fecha all-day + conversión de instantes a `snapshot.timezone` antes de agrupar y presentar | Fixture temporal con cambio de fecha, offsets equivalentes y horario de verano | Mapper implementado; conversión visual pendiente |
...
| KR-REQ-019 | Screensaver sin red; `Leave screen as-is`; composición propia configurable; preview separada y limpieza de campos temporales | `book_screensaver_smoke`, `screensaver_preview_smoke`, `screensaver_integration_smoke` + ciclo UIManager | Implementación local; verificación en dispositivo pendiente |
...
| KR-REQ-031 | `remark-math`/`rehype-katex` en web y Markdown HTML seguro con símbolos, estilos matemáticos y `sup`/`sub` en `markdown.lua` | Test web de KaTeX, smokes de Markdown enriquecido, `markdown_spec.lua` y KOReader real | Implementación local y smokes correctos; Busted/dispositivo pendientes |

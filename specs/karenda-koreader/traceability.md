...
| KR-REQ-002 | Descriptores `karenda_calendar`/`karenda_notes`/`karenda`, con métodos públicos de fallback y superficie unificada | `simpleui_integration_spec.lua`, `simpleui_overlay_smoke.lua`, SimpleUI real + mock de fallback | Implementación local; verificación pendiente |
| KR-REQ-003 | Código web de emparejamiento + token interno por dispositivo + configuración Lua; CORS explícito para web publicada y Android Capacitor | Preflight `OPTIONS` desde `https://karenda.insforge.site` y `https://localhost`, canje autenticado por código y prueba de sincronización | Web/backend implementados; integración pendiente |
...
| KR-REQ-016 | `calendar_view` a pantalla completa con TitleBar, selector persistente en orden Agenda/Mes/Semana/Día, cuerpo desplazable, pie ButtonTable, cuadrícula Mes/Semana, filas Agenda/Día, filtro de completados en Agenda y TextViewer | `calendar_data_spec`, pruebas de agrupación/rollover, filtro pending-only, `calendar_view_smoke` vertical/horizontal y prueba KOReader/Kindle real | Implementación local y smoke correctos; verificación en dispositivo pendiente |
...
| KR-REQ-018 | Dos paquetes con contexto opcional `calendar`, `note`, `none`; políticas configurables en `Wallpaper`; prioridad `Leave screen as-is`, wallpaper local y delegación | `screensaver_policy_spec`, `screensaver_config_spec`, `screensaver_context_smoke`, `screensaver_integration_smoke`, dispositivo real | Implementación local y smokes correctos; verificación en dispositivo pendiente |
| KR-REQ-019 | Wallpaper independiente y sin red; composición minimalista configurable, progreso sin etiqueta redundante y agrupación opcional de páginas/tiempo restantes; preview separada, limpieza de campos temporales y pre-refresco e-ink completo antes del libro | `book_screensaver_smoke`, `screensaver_config_spec`, `screensaver_preview_smoke`, `screensaver_integration_smoke`, instalación aislada + ciclo UIManager y pantalla e-ink simulada | Implementación local y smokes correctos; verificación en dispositivo pendiente |
...
| KR-REQ-026 | Vistas cache-first, superficie unificada y sincronización inicial explícita | Mock local, `calendar_view_smoke.lua`, fixture sin caché y KOReader real | Implementación local; verificación pendiente |
...
| KR-REQ-028 | SVG locales de calendario/notas/Karenda con contorno monocromo de `48x48` | Inspección estática de assets y SimpleUI/Kindle real | Implementación local correcta; verificación en dispositivo pendiente |
...
| KR-REQ-030 | Descriptores SimpleUI in-place asíncronos, indicador temporal, superficie unificada y pantallas `UIManager` sobre el stack actual | Test de descriptors, smoke de callbacks/indicador/toggle y SimpleUI/KOReader real desde Home/Library/Reader | Implementación local y smokes correctos; restauración real del stack pendiente por dispositivo |
| KR-REQ-031 | `remark-math`/`rehype-katex` en web y Markdown HTML seguro con símbolos, `\\ddagger` como `‡`, estilos matemáticos, símbolos lógicos con subíndice, `sup`/`sub` y `1em` de margen más `1em` de padding interno en `ol`/`ul` en `markdown.lua` | Test web de KaTeX, smokes de Markdown enriquecido, `markdown_spec.lua`, `calendar_view_smoke.lua` y KOReader real | Implementación local y smokes correctos; Busted/dispositivo pendientes |
...
| KR-REQ-036 | `karenda-screensaver.koplugin` funciona sin Karenda ni red; `karenda.koplugin` funciona sin wallpaper; puente tolerante, políticas `preserve`/`book` y `delegate`/`as_is`; panel único y métricas ordenables | `screensaver_context_smoke`, `screensaver_policy_spec`, `screensaver_config_spec`, `book_screensaver_smoke`, `screensaver_integration_smoke`, KOReader real con cada instalación | Implementación local y smokes correctos; validación en dispositivo pendiente |
| KR-NFR-001 | Dos paquetes `.koplugin` independientes, con puente opcional y sin dependencia inversa | Inspección del árbol, pruebas de carga aislada y smokes de contexto | Implementado localmente; carga en dispositivo pendiente |
